...
module.exports = {
    description: "Zobrazí základní informace a návod k použití",
    args: false,
        ...
        try {
            const embed = new Discord.EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: 'Použití', value: 'Pomocí příkazu `/pokus [název pokusu]` se vyvolá stránka s informacemi o pokusu. Pokud si nejste jistí, jak se přesně pokus jmenuje, příkaz `/list` zobrazí seznam všech uložených pokusů.\n(např. `/pokus ultrazvuk` zobrazí informace o pokusu "ultrazvuk")', inline: false },
                    { name: 'Správa záznamů (pouze pro editor roli)', value: 'Záznamy pokusů se přidávjí pomocí formuláře, který lze vyvolat příkazem `/pridat`. Systém ignoruje diakritiku a velká písmena při ukládání i vyhledávání.\nPro úpravu záznamu poté slouží příkaz `/upravit [název pokusu]` a pro odstranění `/odebrat [název pokusu]` \nZáznamy se také dají ze systému stáhnout příkazem `/stahnout [název pokusu]`.', inline: false },
